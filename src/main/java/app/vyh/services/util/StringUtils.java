...
public final class StringUtils {
    ...
    static String capitalize(final String word) {
        char[] working = word.toLowerCase().toCharArray();
    ...
    public static String titleCase(final String title) {
        ...
        String[] words = title.toLowerCase().split(WORD_SEPARATOR);
        words[0] = capitalize(words[0]);
        if (words.length > 1) {
            for (int ind = 1; ind < words.length; ind++) {
                if (!ARTICLES.contains(words[ind])) {
                    words[ind] = capitalize(words[ind]);
                }
            }
        }
